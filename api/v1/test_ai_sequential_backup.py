...
def test_ai_services(request):
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    if claude_response['success'] and claude_response['content']:
                        synopsis_result = loop.run_until_complete(
                    ...
                    # Extract token counts for main response
                    ...
                    # Create AIResponse record for main response
                    if ai_query:
                        try:
                            claude_service_obj = AIService.objects.get(name='claude')
                            ...
                            # Create AIResponse record for synopsis generation
        ...
        if 'openai' in services and settings.OPENAI_API_KEY:
            try:
                ...
                try:
                    ...
                    if ai_query:
                        try:
                            openai_service_obj = AIService.objects.get(name='openai')
                            AIResponse.objects.create(
                                query=ai_query,
        ...
        if 'gemini' in services and settings.GEMINI_API_KEY:
            try:
                ...
                try:
                    ...
                    if ai_query:
                        try:
                            gemini_service_obj = AIService.objects.get(name='gemini')
                            AIResponse.objects.create(
                                query=ai_query,
        ...
        # Mark AIQuery as completed if it was created
        if ai_query:
            ai_query.status = 'completed'
            ai_query.completed_at = timezone.now()
            ai_query.total_responses = len(results)
            ai_query.save()

            # Update conversation metadata with latest costs
            if ai_query.conversation:
                ai_query.conversation.update_conversation_metadata()
